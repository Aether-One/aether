...
├── LICENSE                  # MIT license
├── package.json             # Package manifest (name: "aether", version: "0.2.0")
...
│   ├── cleancode.ts         # /cleancode command (review, ignore, paradigm)
...
│   ├── prompt.ts            # /prompt command (optimize developer task into AI prompt)
...
├── genesis/
│   ├── cleancode-heuristics.ts  # Heuristic clean-code scan (regex-based, no AI)
│   ├── cleancode.ts             # Hybrid clean-code review (heuristics + AI), report generation
│   ├── constants.ts             # Env-overridable constants (MAX_FILE_SIZE, MAX_TOTAL_CHARS, etc.)
│   ├── context.ts               # ProjectContext, FileContent, scanContext()
│   ├── digest.ts                # buildPlannerDigest(), detectSignals(), extractSymbols()
│   ├── distill.ts               # distillFilesIncremental(), distill cache logic
│   ├── docs.ts                  # DocDefinition[], DOC_DEFINITIONS (13 docs), buildDocPrompt()
...
│   │   └── tech-stack.ts
│   └── pipeline/
│       ├── cleancode.ts     # CLEANCODE paradigms, buildCleanCodeScanPrompt()
│       ├── optimize.ts      # OPTIMIZE_PROMPT, buildOptimizePrompt()
...
| `src/commands/registry.ts` | **CommandRegistry** — registers `/genesis`, `/sync`, `/config`, `/clean`, `/help`, `/exclude`, `/cleancode`, `/prompt`; parses `/command args` input |
...
| `src/genesis/types.ts` | **Core domain types** — `ProjectContext`, `FileFingerprint`, `DocDefinition`, `Snapshot`, `SyncPlan`, `DocSection` enum, `CleanCodeIssue`, `CleanCodeReport`, `CleanCodeParadigm` |
...
| `src/ui/prompt.ts` | **Interactive REPL** — `startChat()` with readline, slash-command dropdown (`/genesis`, `/config`, `/exclude`, `/cleancode`, `/prompt`, etc.), keyword responses |
...
| `src/commands/cleancode.ts` | **Clean-code command** — `/cleancode review [path] [--yes]`, `/cleancode ignore [pattern]`, `/cleancode paradigm [name]`; hybrid AI+heuristic review with cost confirmation |
| `src/genesis/cleancode.ts` | **Hybrid clean-code review** — `scanCleanCodeHeuristics()`, `scanCleanCodeHybrid()`, `buildCleanCodeReport()`, `buildCleanCodeMarkdown()`; writes `.aether/cleancode-report.md` |
| `src/genesis/cleancode-heuristics.ts` | **Heuristic clean-code scan** — regex-based detection for `long-function`, `deep-nesting`, `magic-number`, `naming`, `duplication`, `dead-code`, `error-handling`, `complexity` |
| `src/prompts/pipeline/cleancode.ts` | **Clean-code prompts** — 4 paradigms (`clean-code`, `solid`, `functional`, `google-style`), `buildCleanCodeScanPrompt()` outputs JSON array of violations |
| `src/prompts/pipeline/optimize.ts` | **Optimize prompt** — `OPTIMIZE_PROMPT`, `buildOptimizePrompt()` turns developer task into optimized prompt for another AI coding assistant |
| `src/commands/prompt.ts` | **Prompt command** — `/prompt <intent>` optimizes a developer task into an AI-ready prompt using project context |
| `src/genesis/constants.ts` | **Env-overridable constants** — `MAX_FILE_SIZE`, `MAX_TOTAL_CHARS`, `MAX_FILES_WALKED`, `MAX_WALK_DEPTH`, `DOC_CONTEXT_BUDGET`, `GEN_CONCURRENCY`, `DISTILL_CONCURRENCY`, `CLEANCODE_CONTEXT_BUDGET` |
| `src/ui/animation.ts` | **Startup animation** — `playStartupAnimation()` (animated logo with stars), `printBanner()` (static fallback) |
| `src/ui/theme.ts` | **Theme constants** — `ACCENT_HEX`, `ACCENT`, `ACCENT_BOLD`, `DIM`, `SUCCESS`, `WARN`, `ERROR` (chalk-based) |
